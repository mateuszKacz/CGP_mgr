###################################################
#####    Class contains main class Gate2D     #####
#####     it is a core item                   #####
#####           of the simulation             #####
###################################################
...
class Net2D:
    ...
    def __init__(self, params):
        ...
        self.net = [[Gate2D(self.params, i, j, operation_type=self.operation_types_dict[i]) for i in
                     range(self.params.hidden_layers_height)] for j in range(self.params.hidden_layers_width)]
    ...
    def show_net(self):
        ...
        for i in range(self.params.hidden_layers_width):
            for j in range(self.params.hidden_layers_height):
                print('Column: %d \t Row: %d \t Active: %s \t Type: %s \t Val: %d' % (i, j, self.net[i][j].active,
                      self.net[i][j].operation_type, self.net[i][j].value))
