...
def main():
    ...
    cgpsa = CGPSA(_gate_func=gate_fun, _obj_func=obj_func.obj_func, _data=data_even_check_gate, _input_data_size=5,
                  _steps=5000, _data_gather_interval=1, _annealing_scheme=['linear'])
    cgpsa.run()
    data_gather.save_to_csv(cgpsa.simulation.get_params_history(), "every_step_momentum_data_evencheck_gate_linear_sa.txt")
