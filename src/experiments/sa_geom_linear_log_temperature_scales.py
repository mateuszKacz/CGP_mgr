...
def main():
    ...
    gate_fun = [gate_func.bin_and, gate_func.bin_nand, gate_func.bin_or,
                gate_func.bin_xor]
    ...
    annealing_param = [x for x in np.logspace(math.log(0.005), math.log(200), 10, base=math.e)]
    max_steps = 3000
    num_sim = 200
    ...
    for scheme in tqdm(schemes, desc='Scheme:'):
        ...
        for temp in tqdm(annealing_param, desc='Temperatures done'):
            data = data_gather.gen_cgp_data(_gate_func=gate_fun,
                                            _obj_func=obj_func.obj_func,
                                            _data=data_even_check_gate,
                                            _input_data_size=5, _size_1d=15,
                                            _num_copies=5, _pdb_mutation=0.06,
