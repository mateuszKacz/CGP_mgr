...
def main():
    ...
    for scheme in tqdm(schemes, desc=f'Schemes'):
        ...
        for steps in tqdm(scheme['max_steps'], 'Max_steps'):
            data = data_gather.gen_cgp_data(_gate_func=gate_fun, _obj_func=obj_func.obj_func,
                                            ...
                                            _annealing_param=annealing_param, _annealing_scheme=scheme['scheme'],
                                            _steps=steps, _num_of_sim=num_sim)
            all_data = all_data + data

        file_path = MAIN_DATA_PATH / f"random2_gate_max_step_10_10000000_{scheme['scheme'][0]}.csv"
