...
class Gate1D:
    ...
    def __init__(self, _params, _gate_func, _gate_index, _value=0.0):
        """
        ...
        self.active_input_value = [0.0, 0.0]
...
class Net1D:
    ...
    def __init__(self, _params, _load_file=False):
        ...
        self.prediction = []
        self.net = [
            Gate1D(self.params, "input", i, _value=0)
            for i in range(self.params.input_data_size)
        ]
        self.net = self.net + [
            Gate1D(
                self.params,
                rnd.choice(self.params.gate_func),
                self.params.input_data_size + i,
            )
    ...
    def init_gates_links(self):
        ...
        for gate_index in range(self.params.input_data_size, len(self.net)):
            for link in range(2):
                ...
                self.net[gate_index].active_input_value[link] = self.net[
                    rand_gate
                ].output_val
    ...
    def show_output(self):
        ...
        print(
            f"Output gate: {self.output_gate_index}  Output _value: "
            f"{self.net[self.output_gate_index].output_val}"
        )
    ...
    def update_gate_value(self, _gate_index):
        ...
        :type _gate_index: int
        """
        self.net[_gate_index].active_input_value[0] = self.net[
            self.net[_gate_index].active_input_index[0]
        ].output_val
        self.net[_gate_index].active_input_value[1] = self.net[
            self.net[_gate_index].active_input_index[1]
        ].output_val
    ...
    def run_data(self, _input_set):
        """
        Method takes one input set of data _input_set and it's iterable to extract
        corresponding output

        :param _input_set: one set of input data (eg. input_gates_num = 5, then
            len(_input_set) = 5 )
    ...
    def mutate(self):
        """
        Method tries to mutate every gate from the Net - it's operation and links
        with probability.
        """

        for gate_index in range(self.params.input_data_size, self.params.total_size):
            self.rnd_link_change(
                gate_index
            )  # attempt to change the links and recalc if needed
            self.net[gate_index].change_operation()
            self.rnd_output_gate()
    ...
    def calculate_total_potential(self):
        ...
        for i in range(len(self.params.data)):
            self.prediction.append(
                self.run_data(self.params.data[i][: self.params.input_data_size])
            )

        self.potential = self.params.obj_func(self.params.data, self.prediction)
        self.output = self.net[
            self.output_gate_index
        ].output_val  # refreshing param value after calculations
